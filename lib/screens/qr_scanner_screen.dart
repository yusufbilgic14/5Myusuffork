...
class _QRScannerScreenState extends State<QRScannerScreen>
  ...
  Future<void> _initializeCamera() async {
    try {
      ...
      if (!mounted) return; // Widget dispose olmuşsa çık / Exit if widget is disposed
      ...
      if (status.isGranted) {
        ...
        if (mounted) {
          setState(() {
            _isPermissionGranted = false;
            _errorMessage = 'Kamera izni kalıcı olarak reddedildi. Ayarlardan izin verin.';
          });
        }
      } else {
        if (mounted) {
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: () async {
                setState(() {
                  _errorMessage = 'Kamera başlatılıyor...'; // Loading durumunu göster / Show loading state
                });
              ...
              child: _errorMessage?.contains('başlatılıyor') == true
                  ...
                  : Text(_errorMessage?.contains('kalıcı') == true ? 'Ayarlara Git' : 'İzin Ver'),
            ),
          ],
        ),
      ),
